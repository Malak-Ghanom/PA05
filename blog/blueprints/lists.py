...
@ lists_bp.route('/newtask', methods=['POST', 'GET'])
def new_task():
...
@ lists_bp.route('/task/view/<int:index>')
def view_task(index):
...
@ lists_bp.route('/task/remove/<int:index>')
...
# edit task
@ lists_bp.route('/task/edit/<int:index>', methods=['GET', 'POST'])
def edit_task(index):
    if request.method == 'GET':
        view_task = tasks[index]
